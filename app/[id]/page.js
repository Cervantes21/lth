...
const getUrl = (MARCA) => {
    const urls = {
        "LTH": "/lth",
        "HI-TEC": "/hitec",
        "LTH EVOLUTION": "/evolution",
        "LTH EVOLUTION AUX": "/auxiliar",
        "LTH PROTECT": "/protect",
        "LTH TAXI": "/taxi",
        "MOTOBATERIA": "/motobaterias"
    };
    return urls[MARCA] || "/";
}
...
export default async function Bateria({ params }) {
    const { id } = await params;
    const decodedId = decodeURIComponent(id);
    const bateria = productos.find(p => p.BCI === decodedId);

    if (!bateria) {
        return (
            <div className="flex flex-col items-center justify-center h-screen">
                <h1 className="text-2xl font-bold">Batería no encontrada</h1>
                <Link href="/" className="text-blue-lth underline mt-4">Volver al inicio</Link>
            </div>
        );
    }

    const lastUrl = getUrl(bateria.MARCA);
    const data = {
        bci: bateria.BCI,
    ...
    return (
        <>
            ...
            <div>
                ...
                <CotizarButton bci={data.bci} marca={bateria.MARCA} />
                
                <Link href={"/garantias-y-ajustes"} className="italic text-center mt-10 mb-20 font-semibold mx-auto w-fit block hover:text-blue-700 underline transition-colors duration-150">
                    Para términos y condiciones consulta la póliza de garantía.
                </Link>
            </div>
